...
Below is an image taken from the [MIDI 1.0 specification](./M1_v4-2-1_MIDI_1-0_Detailed_Specification_96-1-4.pdf). For the purposes of this project, only the `MIDI OUT` circuit is relevant. The `MIDI IN` is assumed to already be present on your audio interface.

<img src="./Images/MIDI/MIDI_standard_hardware.png" width=450px>

### Protocol

MIDI works at serial specifications (see [MIDI 1.0 specification](./Datasheets/M1_v4-2-1_MIDI_1-0_Detailed_Specification_96-1-4.pdf)):
- 31.25 Kbaud
...
This YouTube [video](https://www.youtube.com/watch?v=2BccxWkUgaU) explains it clearly as well.
...
Page 9 of the standard mentions that a `Note-On` message takes 3 data frames (1 status byte and 2 data bytes (note and velocity) = 30 bits) which would mean 960us. So multiple notes at the same time might have some audible delay.

To reduce the time per note, the Running Status can be used. MIDI stays in last status that is was send until a new status is sent. Meaning that if the Note-On status is set, you can send multiple pairs of data bytes to save a byte (the status byte) on each note.

This [video](https://www.youtube.com/watch?v=JXfrqjkVcKg&t=696s) shows that notes are mainly 'turned off' (a velocity of 0) right after sending them. But some modules allow to set an 'on' time. Nevertheless, a hit on the drum needs to be turned on and off. Assuming the running status is used, this requies 1 status byte to start the running note and 4 data bytes to turn on and off the note.
...
The image below (from [wikipedia](https://en.wikipedia.org/wiki/General_MIDI#Percussion)) shows a general mapping. But every brand of electric drum may be using a different mapping.
